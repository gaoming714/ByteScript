...
logConfig("logs/cheat0x01.log", rotation="10 MB", level="DEBUG", lite=True)
...
COOKIE = Path() / "cookies" / "Bot_0001.json"
...
def page_video(page, mid = "7403218550913125671"):
    page.goto("https://www.douyin.com/?modal_id={}&recommend=1".format(mid))
    # ipdb.set_trace()
    ...
    page.mouse.wheel(0, - 1024 * 1024)
    time.sleep(0.5)
    # # click_sugar(page.locator(".TcRmcHfA"))
    # # logger.debug("press K")
    # # page.keyboard.pressx("k")
    # # time.sleep(0.5)
    # # logger.debug("wheel up")
    # # page.mouse.wheel(0, - 1024 * 1024)
    # # time.sleep(0.3)
    logger.debug("click press")
    ...
    logger.warning("open comment manually")
    # ipdb.set_trace()
    page.locator(".kT7icnwc").nth(1).click()
    time.sleep(1)
    page.locator(".dzwAmqeM").hover()
    for _ in range(50):
        page.mouse.wheel(1, 1024)
        time.sleep(0.5)
    comment_els = page.locator(".comment-item-avatar").locator("a.hY8lWHgA")
    href_list = []
    for index in range(comment_els.count()):
        sec_url = comment_els.nth(index).get_attribute("href")
        if "/user/" in sec_url:
            sec_uid = sec_url.split('/user/')[1]
            href_list.append(sec_uid)
    logger.info("Found {}".format(len(href_list)))
    for sec_uid in href_list:
        send_to_home(page, sec_uid)

def send_to_home(page, sec_uid):
    ...
    if page.locator(".public-DraftStyleDefault-block").count():
        ...
        time.sleep(0.5)
        # ipdb.set_trace()
        # msg_str = "榴莲控福利来啦！✨ 榴莲爱好者千万别错过！我们的榴莲，个个都是肉厚核小，香气浓郁，甜到心里！一口下去，简直是味蕾的盛宴！榴莲控们"
        msg_str = ( "新规有效fen步骤\n"
                    "1.点第一条作品完播\n"
                    "2.点赞/评论\n"
                    "3.关注\n"
        ...
        time.sleep(0.5)
        click_sugar(page.locator(".SHV6n6VV"))
        time.sleep(0.5)
