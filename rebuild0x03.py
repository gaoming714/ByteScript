...
COOKIE = Path() / "cookies" / "Bellamy_overseas.json"
# COOKIE = Path() / "cookies" / "cookie_Bellamy_177.json"

main_path = Path() / "data" / "01"
########################################################################################
...
def launch():
    ...
    with sync_playwright() as p:
        ...
        traffic_uni(page, "3703182854402998516", "白金3段800g")
        traffic_uni(page, "3703183502993392021", "白金3段800g")
        traffic_uni(page, "3703184280281809012", "白金3段800g")
        logger.success("manual success done")
        return
        ...
        # fetch_new_pid(page, "【夏日健康风】格力↘空调云轩大1匹1.5匹1级能效时尚挂机大风力")
        # rebuild_product(
        #             page, "3699390584943935717", None, "no"
        #         )
        # ipdb.set_trace()
        # limitsales(page, limit_id,  sale_list, new_pid)
        ...
        for index in range(
            ...
            for retry_times in range(500):
                new_pid = fetch_new_pid(page, ns.payload)
                if new_pid:
                    break
                else:
                ...
                if retry_times == 400:
                    ...
                    raise
                time.sleep(4)
            record_id(main_path / "record.txt", new_pid)
            logger.debug(new_pid)
            kucun(page, new_pid, 2)  # 库存
            # limitsales(page, limit_id, sale_list, new_pid)  # 限时限量购
            # traffic_standard(page, new_pid, uname)  # 标准推广
            traffic_uni(page, new_pid, uname)  # 全域推广
            logger.success("Done {}".format(new_pid))
...
def fetch_new_pid(page, target_title):
    # goto list
...
def rebuild_product(page, copyid, figure, uname=None):
    ...
    user_content = (
        "{}\n"
        ".上一句变换顺序,不改变原意，去掉标点符号,\n"
        ".{} 是一个固定词组,\n"
        ".输出至多 56 个字符 \n"
        ".输出json格式,key='data'".format(full_name_elem.first.input_value(), uname)
    )
    ...
    user_content = (
        ...
        "{} 是一个固定词组,\n"
        "输出至多 24 个字符）\n"
        "输出json格式,key='data'".format(short_name_elem.first.input_value(), uname)
    )
    res_dict = kimiDB.ask(user_content)
...
def kucun(page, pid, quantity = 2):
    ...
    time.sleep(1.5)
    page.locator(".auxo-table-row").locator(".icon").click()
    time.sleep(0.5)
    page.get_by_role("cell", name="可售库存", exact=False).locator("svg").last.click()
    time.sleep(0.5)
    page.locator("[id=\"\\34 PLZWYBBC01_4PLZWYBBC01\"]").fill("{}".format(quantity))
    time.sleep(0.5)
    # final check
    ...
    #check
    # if page.get_by_role("cell", name="可售库存 2", exact=False).count() == 0:
    #     logger.debug("kucun error")
    #     logger.debug("kucun not 2 for pid {}".format(pid))
    #     raise
    # time.sleep(1)
...
def traffic_standard(page, pid_target, uname):
    ...
    time.sleep(1)
    page.get_by_placeholder("请输入金额").fill("500")
    time.sleep(0.5)
    page.get_by_placeholder("请输入支付ROI目标").fill("3.0")
    time.sleep(1)
...
def traffic_uni(page, pid_target, uname):
    ...
    page.goto(
        ...
        "https://qianchuan.jinritemai.com/creation/uni-prom-product?aavid=1795939881415883"  # 贝拉米
    )
    # kimi
    user_content = (
        "给我10个贝拉米奶粉的种草标题.每个标题 30 至 50个汉字,禁止用广告极限词."
    ...
    # ROI & else
    page.get_by_role("textbox", name="请输入目标").fill("1.7")
    time.sleep(0.5)
    ...
    tick = pendulum.now()
    base_info = "{} {}".format(uname, tick.to_iso8601_string()[:-5])
